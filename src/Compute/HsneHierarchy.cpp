...
void HsneHierarchy::initialize(hdps::CoreInterface* core, const Points& inputData, const std::vector<bool>& enabledDimensions, const HsneParameters& parameters) // hard coded to use in test plugin
{
    _core = core;
...
void HsneHierarchy::saveCacheParameters(std::string fileName, const Hsne::Parameters& internalParams) const {
    ...
    // commented to avoid dependency on nlohmann/json
