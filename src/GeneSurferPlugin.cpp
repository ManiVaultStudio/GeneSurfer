...
void GeneSurferPlugin::publishSelection(const QString& selection)
{
    ...
    {
        QString ProjectAveragesRNAonUMAPDatasetName = "Mapped RNA on UMAP";// should correspond to "Mapped RNA on UMAP"
        ...
        if (allDatasets.size()>0)
        {
            for (auto dataset : allDatasets)
            {
                ...
                {
                    DimensionPickerAction* dimensionPickerAction1 = dynamic_cast<DimensionPickerAction*>(dataset->findChildByPath("Settings/Averages Dataset Dimension"));
    ...
    {
        QString ProjectAveragesATAConUMAPDatasetName = "Mapped ATAC on UMAP";// should correspond to "Mapped ATAC on UMAP"
...
void GeneSurferPlugin::updateDimView(const QString& selectedDimName)
{
    ...
    if (_isSingleCell != true) {
        ...
        if (_ATACtoRNA)
            avgValue = _avgExprRNA(Eigen::all, _selectedDimIndex);
        else
            avgValue = _avgExpr(Eigen::all, _selectedDimIndex);

        //populate the avg expr values to all ST points
        dimValues.resize(_numPoints);
#pragma omp parallel for
        for (int i = 0; i < _numPoints; ++i) {
            QString label = _cellLabels[i]; // Get the cluster alias label name of the cell
            dimValues[i] = avgValue(_clusterAliasToRowMap[label]);
        }
...
void GeneSurferPlugin::clusterGenes()
{
    ...
    qDebug() << "filteredDimNames[0]" << filteredDimNames[0];
